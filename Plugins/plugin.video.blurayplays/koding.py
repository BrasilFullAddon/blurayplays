...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Bluray Plays,Erro de codigo - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Bluray Plays,Falha de ao se conectar com o servidor. - "+str(e.reason)+",10000,"+icon+")")

def SKindex():
    addon_log("SKindex")
    getData(off,'')
    msg2 = "\x68\x74\x74\x70\x73\x3a\x2f\x2f\x70\x61\x73\x74\x65\x62\x69\x6e\x2e\x63\x6f\x6d\x2f\x72\x61\x77\x2f\x79\x7a\x44\x51\x30\x76\x45\x56"
    msg = msg2 
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Novos conteúdos adicionados.,5000,"+icon+")")
        if not url is None:
...
def getSearchData(url,icon, data=None):
    ...
    keyboard.setHeading("[COLOR white][B]Bluray[COLOR red]Plays[COLOR white]Pesquisa[/B][/COLOR]")
    keyboard.setDefault('')
    keyboard.doModal()
    if keyboard.isConfirmed():
        ...
        xbmcgui.Dialog().ok('[COLOR white][B]Bluray[COLOR red]Plays[/B][/COLOR]', '[COLOR white][B]Ue, vai pesquisar nada nao?rs[/B][/COLOR]')
        quit()
    fanart=''
    dontLink=False
    os.path.join(home, 'resources', 'fanart.gif')
...
def SearchChannels():
    ...
    progress.create('blurayplays Procurando Espere',' ')
...
base = "http://173.249.26.98/user/"+passwordl+"/base/main.xml"
off =  base
xbmc.log(base)
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Atualize o commonresolvers para continuar. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        xbmc.executebuiltin("XBMC.Notification(blurayplays,Usando Urlresolver.. - ,5000)")
        import urlresolver
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR white][B]Bluray [/B][/COLOR][COLOR red][B]Plays[/B][/COLOR] - Selecione uma Opcao:', names)
            if index >= 0:
                playname=names[index]
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('blurayplays','Escolha um local para salvar os arquivos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('blurayplays', 'Você deseja adicionar esse arquivo como fonte?')
        if ret:
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                name = name + '[COLOR firebrick] (' + str(len(playlist)) + ' Opcoes )[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
